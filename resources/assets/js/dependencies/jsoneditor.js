...
// From MDN
	...
	JSONEditor.defaults.editors.string = JSONEditor.AbstractEditor.extend({
		...
		setValue: function (value, initial, from_template) {
			...
			// If using CKEditor, update the WYSIWYG
			if (this.ckeditor_instance) {
				this.ckeditor_instance.setData(sanitized);
			}
		...
		afterInputReady: function () {
			...
			if (this.source_code) {
				...
				if (this.options.wysiwyg &&
					...
					window.jQuery && window.jQuery.fn && window.ClassicEditor
				) {
					ClassicEditor
						.create(self.input)
						.then((result) => {
	...
	JSONEditor.defaults.themes.bootstrap4 = JSONEditor.AbstractTheme.extend({
		...
		addInputError: function (input, text) {
			...
			if (!input.errmsg) {
				...
				alert.innerHTML = "<strong>Oh snap!</strong> ";
				input.controlgroup.appendChild(alert);
				alert.innerHTML += text;
				input.errmsg = alert;
			} else {
				$(input.controlgroup.lastChild).show();
			}
		},
		removeInputError: function (input) {
			if (!input.errmsg) return;
			$(input.controlgroup.lastChild).hide();
		},
		...
		updateProgressBarUnknown: function (progressBar) {
			...
			const bar = progressBar.firstChild;
			progressBar.className = 'progress progress-bar-striped progress-bar-animated';
			bar.removeAttribute('aria-valuenow');
			bar.style.width = '100%';
			bar.innerHTML = '';
		}
